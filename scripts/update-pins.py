...
Usage:
    ...
    ./scripts/update-pins.py --replay-log FILE         # test hash-fixing against a captured log
"""
...
def parse_fod_mismatches(output: str) -> list[HashMismatch]:
    ...
    Nix emits two consecutive lines like:
         specified: sha256-XXXX
            got:    sha256-YYYY
    """
    pattern = re.compile(r"specified:\s*(sha256-\S+)\s+got:\s*(sha256-\S+)")
    seen: set[HashMismatch] = set()
    for match in pattern.finditer(output):
        old, new = match.group(1).rstrip(","), match.group(2).rstrip(",")
        if old != new:
...
def reset_cargo_hash_content(content: str) -> tuple[str, bool]:
    ...
    """
    new, count = re.subn(r'(cargoHash\s*=\s*)"sha256-[^"]*"', r'\1""', content)
    return new, count > 0
...
def fill_empty_cargo_hash(content: str, new_hash: str) -> tuple[str, bool]:
    ...
    new, count = re.subn(r'(cargoHash\s*=\s*)""', rf'\1"{new_hash}"', content)
    return new, count > 0
...
def find_nix_files(root: Path) -> list[Path]:
    ...
    return sorted(p for p in root.rglob("*.nix") if ".workspace" not in p.parts)
...
def _files_for_pin(nix_files: list[Path], pin: Optional[str]) -> list[Path]:
    ...
    if not pin:
        return nix_files
    candidates = [path for path in nix_files if pin in path.parts]
...
def reset_cargo_hash_in_files(
    ...
    """
    for path in _files_for_pin(nix_files, pin):
        content = path.read_text()
        new_content, changed = reset_cargo_hash_content(content)
        if changed:
            print(f"  Resetting cargoHash in {path.relative_to(root)}")
...
def fix_mismatch_in_files(
    ...
    for path in nix_files:
        ...
        if changed:
            print(f"  Fixing {path.relative_to(root)}: {mismatch.old} -> {mismatch.new}")
            path.write_text(new_content)
            changed_any = True
    ...
    for path in _files_for_pin(nix_files, pin):
        ...
        if changed:
            print(f'  Fixing {path.relative_to(root)}: cargoHash="" -> {mismatch.new}')
            path.write_text(new_content)
            return True
...
def handle_build_failure(
    ...
    for mismatch in mismatches:
        if not fix_mismatch_in_files(nix_files, mismatch, root, pin=pin):
            all_fixed = False
    return all_fixed
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.replay_log:
        ...
        nix_files = find_nix_files(REPO_ROOT)
        print(f"==> Replaying log: {log_path}")
        result = handle_build_failure(output, nix_files, REPO_ROOT)
